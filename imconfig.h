...
// single statement. #define IM_ASSERT(_EXPR)  MyAssert(_EXPR) #define IM_ASSERT(_EXPR)  ((void)(_EXPR))     // Disable
// asserts
...
#define IM_ASSERT(_EXPR)                 \
  do {                                   \
    if (!(_EXPR)) { __asm__("int $3"); } \
  } while (false)
#elif defined(__aarch64__)
#define IM_ASSERT(_EXPR)                 \
  do {                                   \
    if (!(_EXPR)) { __asm__("brk #0"); } \
  } while (false)
#else
#include <signal.h>
#define IM_ASSERT(_EXPR)              \
...
// from. Read "Context and Memory Allocators" section of imgui.cpp for more details. #define IMGUI_API __declspec(
// dllexport ) #define IMGUI_API __declspec( dllimport )
...
// io.AddKeyEvent(). This is automatically done by IMGUI_DISABLE_OBSOLETE_FUNCTIONS.
...
// useful in day to day work. Please read comments in imgui_demo.cpp. #define IMGUI_DISABLE // Disable everything: all
// headers and source files will be empty. #define IMGUI_DISABLE_DEMO_WINDOWS                        // Disable demo
// windows: ShowDemoWindow()/ShowStyleEditor() will be empty. #define IMGUI_DISABLE_DEBUG_TOOLS // Disable
// metrics/debugger and other debug tools: ShowMetricsWindow(), ShowDebugLogWindow() and ShowIDStackToolWindow() will be
// empty.
...
// and ImFileHandle at all (replace them with dummies) #define IMGUI_DISABLE_DEFAULT_FILE_FUNCTIONS // Don't implement
// ImFileOpen/ImFileClose/ImFileRead/ImFileWrite and ImFileHandle so you can implement them yourself if you don't want
// to link with fopen/fclose/fread/fwrite. This will also disable the LogToTTY() function. #define
...
// formats provided by stb_sprintf.h. #define IMGUI_USE_STB_SPRINTF
...
// vcpkg with 'vcpkg install freetype --triplet=x64-windows' + 'vcpkg integrate install'. #define IMGUI_ENABLE_FREETYPE
...
// which is licensed under CeCILL-C Free Software License Agreement) #define IMGUI_ENABLE_FREETYPE_LUNASVG
...
// backend. #define IMGUI_ENABLE_STB_TRUETYPE
